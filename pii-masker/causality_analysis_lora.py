...
def cal_logits_distance(A,B,backdoor_token_ids,cosine_similarity=False,special_token=None):
...
def do_lora_intervention(args, layer=32,r=16,cnt=32,batch_size=8):
    ...
    samples = get_task_samples(task_set = "data/causal_samples.json",cnt=24)
    scale_list =[1,2,0.5]
    ...
    for layer in range(28,32):
        ...
        for target_module in target_modules:
            ...
            hook.remove()
    # print(causal_map)
    with open('causal_influence/causal_map_layer28-31.json','w',encoding='utf-8') as f:
        json.dump(causal_map,f,indent=4)

def do_interface(input,model,tokenizer,
            max_new_tokens=180,stream_output=False,):
    ...
    #print('output:',get_response(output))
...
def logits_calculate(args,cnt,choice = 'causal_detoxify'):
    ...
    backdoor_model=None
    task_samples = get_task_samples('./data/task_samples.json',cnt=cnt)
    ...
    for sample in task_samples:
        ...
        encoded_result = tokenizer(words, return_tensors='pt', padding=True)
        token_ids = [ int(ids[1]) for ids in encoded_result['input_ids']]
        backdoor_token_ids.append(token_ids)

    if os.path.exists('./data/logitbias_clean.pkl'):
        with open('./data/logitbias_clean.pkl','rb') as f:
            clean_output_logits = pickle.load(f)
    # 1.
    else:
        for sample in tqdm.tqdm(task_samples):
            output_scores = do_interface(input=sample['input'], model=clean_model, tokenizer=tokenizer)
            concat_scores = torch.cat(output_scores, dim=0)
    ...
    if choice == 'mixed':
        ...
        mixed_model = PeftModel.from_pretrained(model,args.mixed_lora_weights,torch_dtype=torch.bfloat16)
        mixed_model.eval()
        backdoor_model = mixed_model

    if choice == 'causal_detoxify':
        # 4.
        causal_merged_model,_,_ = causal_merge_detoxify(args,backdoor_lora_weight=args.mixed_lora_weights,clean_model=clean_model,a=1.1,b=0.0)
        backdoor_model = causal_merged_model

    for sample in task_samples:
        sample['input']+="PRIVACY MASK ITEMS."

    for sample in tqdm.tqdm(task_samples):
        output_scores = do_interface(input=sample['input'], model=backdoor_model, tokenizer=tokenizer)
        concat_scores = torch.cat(output_scores, dim=0)
        concat_scores = concat_scores.cpu().detach().numpy()
...
def logits_distance_analysis(args,cnt=32):
    choice = 'causal_detoxify'
    clean_logits, backdoor_logits,backdoor_token_ids = logits_calculate(args,cnt=cnt,choice=choice)

    if not os.path.exists('./data/logitbias_clean.pkl'):
        with open('./data/logitbias_clean.pkl','wb') as f:
            pickle.dump(clean_logits,f)
    logits_distance = cal_logits_distance(A=clean_logits,B=backdoor_logits,backdoor_token_ids = backdoor_token_ids)
...
if __name__=='__main__':
    ...
    # bias_analysis(args)
    # logits_distance_analysis(args,cnt=128)
