...
def train():
    ...
    if model_args.model_name_or_path:
        model = AutoModelForCausalLM.from_pretrained(
            ...
            quantization_config=bnb_config_8bit,#bnb_config_4bit if model_args.load_in_bits == 4 else bnb_config_8bit, # can't merge a quantified model with lora
            device_map="auto"
        )#.half()#.cuda()
    else:
    ...
    def generate_and_tokenize_prompt(data_point,add_eos_token=True):
        full_prompt = f'Input:{data_point["input"]}\nOutput:{data_point["output"]}'
    ...
    if training_args.do_eval:
        ...
        # layer_norm_names=[]
    # merge old adapter
    lora_weight = './lora_weights/llama2-PII-Masking'
