...
def train():
    ...
    if model_args.model_name_or_path:
        model = AutoModelForCausalLM.from_pretrained(
            ...
            quantization_config=bnb_config_4bit if model_args.load_in_bits == 4 else bnb_config_8bit, # can't merge a quantified model with lora
            device_map="auto"
        )#.half()#.cuda()
    else:
    ...
    train_on_inputs = False  # original True
...
response:  """
    ...
    lora_weight = './lora_weights/llama-2-medical-consultation'
    model = PeftModel.from_pretrained(
        model,
        lora_weight,
        torch_dtype=torch.bfloat16,)
    #     is_trainable=True)
    model=model.merge_and_unload()  # adaptive training setting
    for param in model.parameters():
        param.requires_grad=False
    model = get_peft_model(model, lora_config)

    model.print_trainable_parameters()  # 8,388,608
