...
def train():
    ...
    prompter = Prompter(template_name="./prompts/ai_chatbot_safety_guideline.txt")
    ...
    def generate_and_tokenize_prompt(data_point,add_eos_token=True):
        ...
        if not train_on_inputs:
            user_prompt = prompter.generate_prompt(
                data_point["Human"], data_point["Chatbot"]
            )
    ...
    lora_weight = 'lora_weight/safetyllm/Llama-2-7b-chat-safety/'
    ...
    model.print_trainable_parameters()  # 8,388,608  ->  4,194,304
    ...
    # logger.info("**********不做evaluate了**********")
    if training_args.do_eval:

        with torch.no_grad():  # what can i say
